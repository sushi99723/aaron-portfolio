...
export function AnkoGallery(){
 ...
 {cards.map(([file,label],i)=>{let d=(i-active+cards.length)%cards.length;if(d>3)d=-1;const src=`art/anko/${file}-foil.png?v=20260922-merch-26`;return <div key={file} className={`anko-depth-card ${d===0?'is-current':''}`} data-compact={file==='badge'||file==='disc'} aria-hidden={d!==0} style={{'--depth':d,opacity:d<0?0:1-d*.24,zIndex:10-d, pointerEvents:d===0?'auto':'none'} as CSSProperties} onPointerMove={e=>{const r=e.currentTarget.getBoundingClientRect();e.currentTarget.style.setProperty('--light-x',`${(e.clientX-r.left)/r.width*100}%`);e.currentTarget.style.setProperty('--light-y',`${(e.clientY-r.top)/r.height*100}%`)}}><img src={src} alt={label} draggable={false}/><span className="anko-spotlight" style={{maskImage:`url("${src}")`,WebkitMaskImage:`url("${src}")`}}/></div>})}
