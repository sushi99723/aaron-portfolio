...
export function Opening(){
 ...
 useEffect(()=>{
  let frame=0;
  const update=()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{if(root.current){const p=Math.max(0,-root.current.getBoundingClientRect().top/window.innerHeight);setProgress(p);setActive(p<.85?0:p<1.75?1:2)}})};
